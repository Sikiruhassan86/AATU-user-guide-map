...
function routeOnCampusNetwork(destinationCoord, mode = 'walking') {
  ...
  const snappedStart = findNearestNode(userLocation);
  let snappedEnd = findNearestNode(destinationCoord);
...
if (!snappedEnd && graphNodes.length) {
  ...
  const path = shortestPath(snappedStart, snappedEnd);
